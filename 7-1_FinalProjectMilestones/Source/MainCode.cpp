...
int main(int argc, char* argv[])
{
    ...
    while (!glfwWindowShouldClose(g_Window))
    {
        ...
        glm::mat4 lightView = glm::lookAt(glm::vec3(0.0f, 8.0f, 5.0f), glm::vec3(0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
        glm::mat4 lightSpaceMatrix = lightProjection * lightView;

        g_SceneManager->RenderDepthPass(lightSpaceMatrix);
// Reset framebuffer and viewport
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
        glViewport(0, 0, g_ViewManager->GetWindowWidth(), g_ViewManager->GetWindowHeight());
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

        // Pass lightSpaceMatrix to main shader
        g_SceneManager->RenderScene(lightSpaceMatrix);
 ...
 *  processInput()
 *
 *  Handles keyboard input including projection toggle.
 ***********************************************************/
void processInput(GLFWwindow* window)
{
    // Exit application
 ...
 *
 *  Updates projection when window is resized.
 ***********************************************************/
void framebuffer_size_callback(GLFWwindow* window, int width, int height)
{
