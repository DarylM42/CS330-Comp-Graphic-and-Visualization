...
public:
	...
	// Set the position of the main light source
	inline void SetLightPosition(const glm::vec3& position) const
	{
		setVec3Value("light.position", position);
	}

	// Set the color of the main light source
	inline void SetLightColor(const glm::vec3& color) const
	{
		setVec3Value("light.color", color);
	}

	// Set the ambient strength of the main light source
	inline void SetAmbientStrength(float strength) const
	{
		setFloatValue("light.ambientStrength", strength);
	}


	inline void SetViewMatrix(const glm::mat4& view) const
	{
		setMat4Value("view", view);
	}

	inline void SetProjectionMatrix(const glm::mat4& projection) const
	{
		setMat4Value("projection", projection);
	}
};